	...
	public static Date toDateNull(Object value) {
		...
		if ((value instanceof Integer)||(value instanceof Long)) {
			try {
				...
				logger.error("String转换成Date报错", e);
				return null;
			}
		}
		...
		if ((value instanceof String)&&(value.toString().trim().length()>0)) {
			if (datepattern.matcher(value.toString()).matches()) {
				try {
					return DateUtil.StringToDate(value+" 00:00:00", DateUtil.h_yyyy_MM_dd_HH_mm_ss);
				} catch (Exception e) {
					logger.error("String转换成Date报错", e);
				}
				try {
					return DateUtil.StringToDate(value+" 00:00:00", DateUtil.x_yyyy_MM_dd_HH_mm_ss);
				} catch (Exception e) {
					logger.error("String转换成Date报错", e);
				}
				return null;
				...
				try {
					return DateUtil.StringToDate(value.toString(), DateUtil.h_yyyy_MM_dd_HH_mm_ss);
				} catch (Exception e) {
					logger.error("String转换成Date报错", e);
				}
				try {
					return DateUtil.StringToDate(value.toString(), DateUtil.x_yyyy_MM_dd_HH_mm_ss);
				} catch (Exception e) {
					logger.error("String转换成Date报错", e);
				}
				return null;
